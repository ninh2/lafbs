...
public void addUser(User newUser) throws SQLException {
    ...
    try (PreparedStatement stmt = cnx.prepareStatement(sql)) {
        ...
        logger.info("Nombre de lignes insérées : " + rowsAffected);

        if (rowsAffected == 0) {
            logger.warning("Aucune ligne insérée. Vérifiez les données et la connexion à la base.");
        }
    } catch (SQLException e) {
        logger.log(Level.SEVERE, "Erreur lors de l'ajout de l'utilisateur", e);
        throw e;  // Relancer l'exception pour la remontée dans la servlet
    }
}
